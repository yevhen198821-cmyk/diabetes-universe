...
test.describe('medical events handlers', { concurrency: 1 }, () => {
  ...
  test('same idempotency key under different account does not collide', async () => {
    ...
    const first = await createSampleEvent(
      'acct-idem-a',
      'shared-idem-key',
      body,
    );
  ...
  test('additive response compatibility preserves required fields', async () => {
    ...
    const insulinCreated = await createSampleEvent(
      'acct-insulin-wave-4e',
      'insulin-wave-4e-create',
      JSON.stringify({ event: semanticEvent }),
    );
    assert.deepEqual(insulinCreated.event, semanticEvent);

    const getResponse = await handleGetMedicalEvent(
      createRequest(medicalEventsUrl(`/${insulinCreated.resourceId}`), {
        headers: authHeaders('acct-insulin-wave-4e'),
      }),
      insulinCreated.resourceId,
    );
    assert.equal(getResponse.status, 200);
    assert.deepEqual((await getResponse.json()).event, semanticEvent);
    ...
    assert.deepEqual(listed.items[0].event, semanticEvent);

    const patchResponse = await handleUpdateMedicalEvent(
      createRequest(medicalEventsUrl(`/${insulinCreated.resourceId}`), {
        method: 'PATCH',
        headers: {
          ...authHeaders('acct-insulin-wave-4e'),
          'if-match': `"${insulinCreated.revision}"`,
          'content-type': 'application/json',
        },
    ...
    assert.deepEqual(updated.event, updatedEvent);
    assert.notEqual(updated.revision, insulinCreated.revision);
  });
