...
export function ProfileMenuDiabetesLinkRow({
  labels,
}: {
  readonly labels: ProfileLabels;
}) {
  return (
    <Link className={profileInteractiveLinkClassName} href="/account/diabetes">
      <span className="flex min-w-0 flex-1 items-start gap-3">
        <span
          className={`grid size-10 shrink-0 place-items-center rounded-xl ${PROFILE_ICON_TONE_CLASS.violet}`}
        >
          <Activity aria-hidden="true" size={18} strokeWidth={2.2} />
        </span>
        <span className="min-w-0 flex-1">
          <span className="text-text-primary block text-sm font-semibold">
            {labels.menu.diabetesSettings.title}
          </span>
          <span className="text-text-secondary mt-0.5 block text-xs">
            {labels.menu.diabetesSettings.subtitle}
          </span>
        </span>
      </span>
