#!/usr/bin/env node
/**
 * Generates official Diabetes Universe app icon SVG assets.
...
const pngTargets = [
  ['app-icon-master.svg', 'app-icon-master-1024.png'],
  ['app-icon-dark.svg', 'app-icon-dark-1024.png'],
  ['app-icon-light.svg', 'app-icon-light-1024.png'],
  ['app-icon-transparent.svg', 'app-icon-transparent-1024.png'],
];

for (const [svgName, pngName] of pngTargets) {
  const svgPath = join(publicDir, svgName);
  const pngPath = join(publicDir, pngName);
  const docsPngPath = join(docsDir, pngName);
  execSync(
    `python3 -m cairosvg "${svgPath}" -o "${pngPath}" -W ${SIZE} -H ${SIZE}`,
    { stdio: 'inherit' },
  );
