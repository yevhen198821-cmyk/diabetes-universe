...
export function classifyMagicLinkError(error: unknown): {
  ...
  const code =
    ...
    /^(?:42P01|42703|28P01|42501|23505|08006|08001|57P01|ENOTFOUND|ECONNREFUSED|ETIMEDOUT)$/.test(
      rawCode,
    )
      ? rawCode
      : undefined;
  ...
  if (/auth email delivery failed/i.test(message)) {
    ...
    /relation .* does not exist|column .* does not exist|42P01|42703/i.test(
      message,
    )
  ) {
    category = 'database_schema';
  } else if (
    /password authentication failed|authentication failed for user|28P01/i.test(
      message,
    )
  ) {
    category = 'database_credentials';
  } else if (
    /connection refused|connection terminated|fetch failed|ENOTFOUND|ECONNREFUSED|ETIMEDOUT/i.test(
      message,
    )
  ) {
    category = 'connection';
  } else if (
    /invalid origin|untrusted origin|invalid callback|csrf/i.test(message)
  ) {
